...
#define trace_adr         0x07     // 0-default( mainmenu ), 2 - boiling, 3 - mashing
...
#define twiceBoilTemp_adr  37      // Start temperature of boiling*2,  shoul be divided to 2 to get correct temperature of boiling
...
void mainmenu() {
  ...
  int  b_Mashing, b_Boiling, b_Service, b_SystemS, b_seeLog; // buttons IDs
  ...
  b_seeLog    = myButtons.addButton(170, 30, 140, 60, "see Log");
  b_Service   = myButtons.addButton(170, 100, 140, 60, "Service");
  b_SystemS   = myButtons.addButton(170, 170, 140, 60, "System");
...
void mashing_setup() {
  int b_TimeMinus[6], b_TimePlus[6], b_TempMinus[6], b_TempPlus[6], b_Menu, b_Mash, b_RestMinus, b_RestPlus, b_prehDeMinus, b_prehDePlus, b_Next; //buttons IDs
  int i;                                                                 //counter
  ...
  myGLCD.printNumI(EEPROM.read(rNum_adr), 260, 50);
  myGLCD.print("Preheat delta", 0, 100);
  myGLCD.printNumI(EEPROM.read(preheatDelta_adr), 260, 100);

  b_RestMinus = myButtons.addButton(220, 50 + i * 22, 20, 20, "-");      // buttons initialisation
  b_RestPlus  = myButtons.addButton(290, 50 + i * 22, 20, 20, "+");
  b_prehDeMinus  = myButtons.addButton(220, 100, 20, 20, "-");
  b_prehDePlus   = myButtons.addButton(290, 100, 20, 20, "+");
...
MashingNext:                                                         // if NEXT pessed
  ...
  for (i = 0; i < EEPROM.read(rNum_adr); i++) {                      // -----=========  drawing menu for choosing temperature and duration of rests =====-------
    myGLCD.print("Rest", 0, 50 + i * 22);                            // printing string by string
    myGLCD.printNumI(i + 1, 64, 50 + i * 22);
    
    myGLCD.printNumI(EEPROM.read(rTemp_startAdr+i), 122, 50 + i * 22, 2);    
    myGLCD.printNumI(EEPROM.read(rTime_startAdr+i), 250, 50 + i * 22, 2);


    b_TempMinus[i]   = myButtons.addButton(90, 50 + i * 22, 20, 20, "-"); // + - buttons initialisation string by string
    b_TempPlus[i]    = myButtons.addButton(165, 50 + i * 22, 20, 20, "+");
    b_TimeMinus[i]   = myButtons.addButton(215, 50 + i * 22, 20, 20, "-"); 
    b_TimePlus[i]    = myButtons.addButton(290, 50 + i * 22, 20, 20, "+");
  }
...
void boiling_setup() {
  int b_TimeMinus[6], b_TimePlus[6], b_Menu, b_Boil, b_hopMinus, b_hopPlus, b_boiltMinus, b_boiltPlus, b_Next; //buttons IDs
  
  int i, k;                                                   // counter
  topic = "BOIL setp";                                     // Topic
  myGLCD.clrScr();                                         // clear screen
  myButtons.deleteAllButtons();                            // delete old buttons
  myButtons.setTextFont(BigFont);                          // buttons font
  myGLCD.setFont(BigFont);                                 // text font
  ...
  myGLCD.print("Start boil at", 0, 100);
  
  myGLCD.printNumI(EEPROM.read(hNum_adr), 250, 50, 2);
  boilingTemp = EEPROM.read(twiceBoilTemp_adr) * 0.5;      //read boil temp form EEPROM
  myGLCD.printNumF(boilingTemp, 1, 235, 100, '.', 4);
  
  b_hopMinus     = myButtons.addButton(210, 50 , 20, 20, "-");
  b_hopPlus      = myButtons.addButton(300, 50 , 20, 20, "+");
  b_boiltMinus   = myButtons.addButton(210, 100, 20, 20, "-");
  b_boiltPlus    = myButtons.addButton(300, 100, 20, 20, "+");
  b_Menu         = myButtons.addButton(30, 195, 120, 40, "Menu");
  b_Next         = myButtons.addButton(170, 195, 120, 40, "Next");
  myButtons.setButtonColors(VGA_WHITE, VGA_GRAY, VGA_BLUE, VGA_RED, VGA_BLUE);
  myButtons.drawButtons();
  ...
  while (true) {                                         //read buttons for choosing N of hops
    ...
    if (myTouch.dataAvailable()) {
      ...
      if (pressed_button == b_boiltMinus and boilingTemp > 90 ) { // - boil temp pressed
        boilingTemp = boilingTemp - 0.5;
        myGLCD.printNumF(boilingTemp, 1, 235, 100, '.', 4);
        EEPROM.update(twiceBoilTemp_adr, round(boilingTemp * 2));
      }
      if (pressed_button == b_boiltPlus and boilingTemp < 99.5 ) { // + boil temp pressed
        boilingTemp = boilingTemp + 0.5;
        myGLCD.printNumF(boilingTemp, 1, 235, 100, '.', 4);
        EEPROM.update(twiceBoilTemp_adr, round(boilingTemp * 2));
      }   
...
BoilingSetNext:
  ...
  for (i = 0; i <= EEPROM.read(hNum_adr); i++) {        // drawing string by string to choose hop times
    ...
    myGLCD.printNumI(EEPROM.read(hTime_startAdr + i), 110, 50 + i * 22, 3); //on minute
    myGLCD.printNumI(EEPROM.read(hTime_startAdr + EEPROM.read(hNum_adr)) - EEPROM.read(hTime_startAdr + i), 230, 50 + i * 22, 3); //minutes to end
    
    b_TimeMinus[i]   = myButtons.addButton(85, 50 + i * 22, 20, 20, "-"); // + - buttons string by string
    b_TimePlus[i]    = myButtons.addButton(165, 50 + i * 22, 20, 20, "+");
  }
  myGLCD.print("End ", 0, 50 + EEPROM.read(hNum_adr) * 22);                     // reprint last string because last hop time is finish time
  myGLCD.setColor(50, 50, 255);                                  // text color Blue
  myGLCD.print("  on minute | to end", 0, 30);
  ...
  while (true) {                                                 //read buttons for choosing time for hops and finish time
    ...
    if (myTouch.dataAvailable()) {
      pressed_button = myButtons.checkButtons();
      for (i = 0; i < EEPROM.read(hNum_adr); i++) {                             //check +- buttons exept last string
        if (pressed_button == b_TimeMinus[i] and EEPROM.read(hTime_startAdr + i) > 0 ) { // - Time pressed
          ...
          myGLCD.printNumI(EEPROM.read(hTime_startAdr + i), 110, 50 + i * 22, 3);
          myGLCD.printNumI(EEPROM.read(hTime_startAdr + EEPROM.read(hNum_adr)) - EEPROM.read(hTime_startAdr + i), 230, 50 + i * 22, 3);
        }
      ...
      i = EEPROM.read(hNum_adr);
      if (pressed_button == b_TimeMinus[i] and EEPROM.read(hTime_startAdr + i) > 0 ) {   // Whole duration of boilind changed, update "to end" time for every hop
        EEPROM.write(hTime_startAdr + i, EEPROM.read(hTime_startAdr + i) - 1);
        for (k = 0; k <= EEPROM.read(hNum_adr); k++) {        
          myGLCD.printNumI(EEPROM.read(hTime_startAdr + k), 110, 50 + k * 22, 3);       //on minute
          myGLCD.printNumI(EEPROM.read(hTime_startAdr + EEPROM.read(hNum_adr)) - EEPROM.read(hTime_startAdr + k), 230, 50 + k * 22, 3); //minutes to end
        }
        
      }
      if (pressed_button == b_TimePlus[i] and EEPROM.read(hTime_startAdr + i) < 120 ) { 
        EEPROM.write(hTime_startAdr + i, EEPROM.read(hTime_startAdr + i) + 1);
        for (k = 0; k <= EEPROM.read(hNum_adr); k++) {        
          myGLCD.printNumI(EEPROM.read(hTime_startAdr + k), 110, 50 + k * 22, 3);       //on minute
          myGLCD.printNumI(EEPROM.read(hTime_startAdr + EEPROM.read(hNum_adr)) - EEPROM.read(hTime_startAdr + k), 230, 50 + k * 22, 3); //minutes to end
...
void boil() {
  int b_Menu,b_powerMinus, b_powerPlus, i, c, p;  // buttons IDs and counters
  int boilingPower = 100;					// heater power rate during boiling 0...100%
  ...
  myButtons.setTextFont(BigFont);            // buttons font 
  b_powerMinus = myButtons.addButton(0, 180, 120, 50, "-");
  b_powerPlus  = myButtons.addButton(140, 180, 120, 50, "+");
  myButtons.setButtonColors(VGA_WHITE, VGA_GRAY, VGA_BLUE, VGA_RED, VGA_BLUE);
  myButtons.drawButtons();
...
restore_boiling:
	...
	myGLCD.print("   % of heater power", 0, 130 );  
    myGLCD.printNumI(boilingPower, 0 , 130, 3);
    
    myButtons.enableButton(b_powerMinus, true);
    myButtons.enableButton(b_powerPlus, true);
    ...
    while (EEPROM.read(hTime_startAdr + i) > pass_minutes) {        // check time for next hop portion
        ...
        if (myTouch.dataAvailable()) {
            pressed_button = myButtons.checkButtons();
            if (pressed_button == b_powerMinus and boilingPower > 10 ) { // - boil power pressed
                boilingPower = boilingPower - 10;
                myGLCD.printNumI(boilingPower, 0, 130, 3);
            }
            if (pressed_button == b_powerPlus and boilingPower < 100 ) { // + boil power pressed
                boilingPower = boilingPower + 10;
                myGLCD.printNumI(boilingPower, 0, 130, 3);
            }
        }

        if (p*10 <= boilingPower) {
          digitalWrite(HEAT_BOILER_PIN, HIGH);    // switch-ON the heater
          heaterStatus=1;
		    } else {
          digitalWrite(HEAT_BOILER_PIN, LOW);    // switch-off the heater
          heaterStatus=0;
        ...
        myGLCD.print("Passed    of    mins", CENTER, 90); //
        myGLCD.printNumI(pass_minutes, 96, 90, 3);      // print passed time
        myGLCD.printNumI(EEPROM.read(hTime_startAdr + EEPROM.read(hNum_adr)), 192, 90, 3);  // print total time of boiling
    ...
    myGLCD.print("Hop #   on    minute", 0, 30);
    myGLCD.printNumI(i + 1, 80, 30);
	
	myGLCD.printNumI(EEPROM.read(hTime_startAdr + i), 160, 30, 3);
    ...
    myButtons.disableButton(b_powerMinus, true);
    myButtons.disableButton(b_powerPlus, true);
  ...
  rwi2c.put(trace_adr, 0);                           // indicate sucsessfull finish of boiling process for power monitor
  
  myGLCD.clrScr();
  myButtons.deleteAllButtons();
  myGLCD.print("    BOILING DONE    ", 0, 110);

  b_Menu = myButtons.addButton(100, 195, 120, 40, "Menu"); //final buttons after boiling done
...
void systemS() {
  ...
  myGLCD.print("Preheat delta", 0, 100);
  myGLCD.print("Start boil at", 0, 130);
  ...
  boilingTemp = EEPROM.read(twiceBoilTemp_adr) * 0.5;      //read boil temp form EEPROM
  myGLCD.printNumF(boilingTemp, 1, 235, 130, '.', 4);

  b_hMinus       = myButtons.addButton(212, 40, 20, 20, "-"); // buttons
  b_hPlus        = myButtons.addButton(300, 40, 20, 20, "+");
  b_mMinus       = myButtons.addButton(212, 70, 20, 20, "-");
  b_mPlus        = myButtons.addButton(300, 70, 20, 20, "+");
  b_prehDeMinus  = myButtons.addButton(212, 100, 20, 20, "-");
  b_prehDePlus   = myButtons.addButton(300, 100, 20, 20, "+");
  b_boiltMinus   = myButtons.addButton(212, 130, 20, 20, "-");
...
void seeLog() {
  ...
  myGLCD.setBackColor(0, 0, 0);                                 // back color black
  myGLCD.setColor(0, 0, 100);                               // text color Blue
  for (i = 20; i < 320; i = i + 20) {
    myGLCD.drawLine(i, 30, i, 239); // draw vertical grid
  }
  myGLCD.setColor(100, 100, 255);    
  myGLCD.drawLine(140, 30, 140, 239); // draw vertical 1h line
  myGLCD.drawLine(260, 30, 260, 239); // draw vertical 2h line
  ...
  for (i = 20; i < 320; i = i + 120) {                           // print time grid
    myGLCD.printNumI((i - 20) / 120, i, 225);
    myGLCD.print("h", i + 8, 225);
  }

  for (i = 0; i < 150; i++)   {
    myGLCD.drawPixel(i*2+20, 240 - 2 * EEPROM.read(startLog_adr + i));  //draw temperature log
	myGLCD.drawPixel(i*2+21, 240 - 2 * EEPROM.read(startLog_adr+ i));
  }
...
void setup() {
  ...
  if (EEPROM.read(preheatDelta_adr) > 5)  {
    EEPROM.write(preheatDelta_adr, 2); //overheating temperature delta before adding malt
  }

/*
  boilingTemp = 25;          //--------==========  temporary code  ==========-------------
  EEPROM.update(twiceBoilTemp_adr, round(boilingTemp * 2));
*/

  boilingTemp= EEPROM.read(twiceBoilTemp_adr)*0.5; //-------===========  main code  ============------------
  if (boilingTemp<95 or boilingTemp>99.5){
       boilingTemp=99.5;
       EEPROM.update(twiceBoilTemp_adr, round(boilingTemp*2));
   }
